<?php
/**
 * Update all employees: Filipino male names, male gender, assign mock photos.
 *
 * Usage: php scripts/update-employees-filipino.php
 */
...
// Source photo files (20 photos, will cycle)
$photoFiles = [
...
$sourceDir = __DIR__ . '/../employee-photos-mock';
$employees = \App\Models\Employee::with('profile')->orderBy('id')->get();

echo "Updating " . $employees->count() . " employees...\n\n";

$i = 0;
foreach ($employees as $employee) {
    $profile = $employee->profile;
    if (!$profile) {
        echo "SKIP ID {$employee->id} ({$employee->employee_number}) — no profile\n";
        $i++;
        continue;
    }

    if ($i >= count($filipinoNames)) {
        echo "SKIP ID {$employee->id} — ran out of names\n";
        continue;
    }
    ...
    Storage::disk('public')->put($destPath, $photoContent);
    ...
    $newName = $name['first_name'] . ' ' . $name['last_name'];
    echo sprintf(
        "ID %2d | %s | %s → %s | photo: %s\n",
        $employee->id,
...
echo "\nDone! Updated {$i} employees.\n";
echo "Photos stored in: storage/app/public/employees/*/profile.jpg\n";
