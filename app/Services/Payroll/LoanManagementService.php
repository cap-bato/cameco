    ...
    public function createLoan(Employee $employee, array $data, User $creator): EmployeeLoan
    {
        ...
        try {
            ...
            $loan = EmployeeLoan::create([
                ...
                'interest_amount' => (float) (($data['amount'] * $interestRate * $data['number_of_months']) / 1200),
                'total_loan_amount' => (float) ($data['amount'] + (($data['amount'] * $interestRate * $data['number_of_months']) / 1200)),
                'number_of_installments' => (int) $data['number_of_months'],
                'installment_amount' => (float) $monthlyPayment,
                'installments_paid' => 0,
                'total_paid' => 0,
                'remaining_balance' => (float) ($data['amount'] + (($data['amount'] * $interestRate * $data['number_of_months']) / 1200)),
                'loan_date' => $startDate,
                'first_deduction_date' => $startDate,
                'status' => 'active',
